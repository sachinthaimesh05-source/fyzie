...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-slate-950/80 backdrop-blur-xl border-b border-white/10 shadow-lg shadow-black/20">
      {/* Top Reading Progress Bar */}
      <div className="w-full h-1 bg-slate-900 overflow-hidden">
        <div 
          className="h-full bg-gradient-to-r from-cyan-500 via-indigo-500 to-purple-500 transition-all duration-300 ease-out"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 group cursor-pointer" onClick={onOpenAuthorModal}>
            <div className="relative w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500/20 via-indigo-500/20 to-purple-500/20 border border-cyan-500/30 flex items-center justify-center p-1 shadow-inner shadow-cyan-500/20 group-hover:border-cyan-400 transition-all duration-300">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="text-lg sm:text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-sky-300 to-indigo-400 tracking-tight group-hover:from-cyan-300 group-hover:to-indigo-300 transition-colors">
                  FyZie
                </span>
                <span className="hidden sm:inline-flex text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-400">
                ...
                <span className="text-cyan-400 font-semibold font-mono">107 Chapters</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center flex-1 max-w-sm mx-4">
          <button
            ...
            className="w-full flex items-center justify-between px-3.5 py-2 rounded-xl bg-slate-900/70 hover:bg-slate-800/80 border border-white/10 hover:border-cyan-500/40 text-slate-400 hover:text-slate-200 text-xs transition-all duration-200 shadow-inner group"
          >
            <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Font Size Adjuster */}
          <div className="hidden sm:flex items-center bg-slate-900/80 p-1 rounded-xl border border-white/10">
            <button
              type="button"
              onClick={() => onChangeFontSize('sm')}
              title="Small text"
              className={`px-2 py-0.5 text-xs font-mono rounded-lg transition-all ${
                fontSize === 'sm' ? 'bg-cyan-500/20 text-cyan-300 font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              A-
            </button>
            <button
              ...
              title="Default text"
              className={`px-2 py-0.5 text-xs font-mono rounded-lg transition-all ${
                fontSize === 'md' ? 'bg-cyan-500/20 text-cyan-300 font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              A
            </button>
            <button
              ...
              title="Large text"
              className={`px-2 py-0.5 text-xs font-mono rounded-lg transition-all ${
                fontSize === 'lg' ? 'bg-cyan-500/20 text-cyan-300 font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              A+
            </button>
          ...
          <button
            ...
            className="hidden lg:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-300 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-500/30 transition-all duration-200"
          >
