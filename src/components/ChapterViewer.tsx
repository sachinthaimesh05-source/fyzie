    ...
    <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 selection:bg-sky-500/30 selection:text-white">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-6 border-b border-white/10">
        <div className="flex items-center gap-2 text-xs">
          <span className="px-2.5 py-1 rounded-lg bg-sky-500/10 text-sky-400 font-mono font-semibold border border-sky-500/20">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={handleShare}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/95 hover:bg-slate-900 text-slate-300 text-xs border border-white/10 transition-colors"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs border transition-all ${
              isSpeaking
                ...
                : 'bg-slate-900/95 text-slate-300 border-white/10 hover:bg-slate-900'
            }`}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${
              isRead 
                ...
                : 'bg-slate-900/95 text-slate-300 hover:text-white border-white/10 hover:border-emerald-500/30'
            }`}
          >
            {isRead ? (
              <>
      ...
      <header className="mt-8 mb-10">
        ...
        <p className="text-base sm:text-lg text-sky-400 font-medium mt-1.5 font-sans">
          {chapter.englishTitle}
        </p>

        {/* Chapter Summary Card with Solid 95% Opacity */}
        <div className="mt-6 p-5 rounded-2xl bg-slate-900/95 backdrop-blur-xl border border-white/10 shadow-xl">
      ...
      {chapter.learningObjectives && chapter.learningObjectives.length > 0 && (
        <section className="my-8 p-6 rounded-2xl bg-slate-900/95 backdrop-blur-xl border border-sky-500/20 shadow-lg shadow-sky-500/5">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {chapter.learningObjectives.map((obj, index) => (
              <li key={index} className="flex items-start gap-2.5 text-xs sm:text-sm text-slate-300">
                <div className="w-4 h-4 rounded-full bg-sky-500/20 border border-sky-500/40 flex items-center justify-center flex-shrink-0 mt-0.5 text-sky-300 text-[10px] font-mono font-bold">
                  ✓
                </div>
          ...
          <section key={sIdx} id={`section-${chapter.id}-${sIdx}`} className="space-y-4 scroll-mt-24">
            ...
            <div className="space-y-3">
              {section.content.map((paragraph, pIdx) => {
                ...
                if (isAnalogy) {
                  return (
                    <div key={pIdx} className="my-4 p-4 rounded-xl bg-violet-950/40 border-l-4 border-violet-400 text-slate-200 text-sm leading-relaxed">
            ...
            {section.asciiDiagram && (
              <div className="my-6 rounded-2xl overflow-hidden border border-sky-500/30 bg-slate-950/95 p-4 shadow-xl">
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/10 text-xs font-mono text-sky-400">
                  <span>පද්ධති ආකෘතිය (Architectural Flow Diagram)</span>
                  <span className="text-[10px] text-slate-500 font-mono">ASCII Schema</span>
                </div>
                <pre className="font-mono text-xs sm:text-sm text-sky-300/90 overflow-x-auto p-2 leading-relaxed whitespace-pre">
                  {section.asciiDiagram}
                </pre>
              </div>
            )}
      ...
      {chapter.keyPoints && chapter.keyPoints.length > 0 && (
        <div className="my-10 p-6 rounded-2xl bg-slate-900/95 backdrop-blur-xl border border-violet-500/20 shadow-xl">
          <div className="flex items-center gap-2.5 mb-4 text-violet-300">
      ...
      {chapter.exercises && chapter.exercises.length > 0 && (
        <div className="my-10 p-6 rounded-2xl bg-slate-900/95 backdrop-blur-xl border border-white/10 shadow-xl">
          <div className="flex items-center gap-2.5 mb-5 text-sky-400">
          ...
          <div className="space-y-4">
            {chapter.exercises.map((ex) => {
              ...
              return (
                <div key={ex.id} className="p-4 rounded-xl bg-slate-950/95 border border-white/10 space-y-3">
                  ...
                  {isAnswerRevealed && (
                    <div className="p-3 rounded-lg bg-slate-900/95 border border-sky-500/30 text-xs text-sky-200/90 leading-relaxed animate-in fade-in duration-200">
      ...
      {/* Official PDF Purchase Promotion Banner (Security & Monetization) - Amber reserved */}
      <div className="my-12 relative overflow-hidden rounded-3xl p-6 sm:p-8 bg-slate-900/95 backdrop-blur-2xl border border-amber-500/40 shadow-2xl">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 flex-shrink-0">
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-3 rounded-2xl bg-slate-900/95 hover:bg-slate-850 text-amber-300 font-semibold text-xs border border-amber-500/40 hover:border-amber-300 transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10"
            >
